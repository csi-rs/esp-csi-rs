...
The engine behind [`esp-csi-rs`](https://github.com/csi-rs/esp-csi-rs): all the CSI collection logic for **ESP32** series devices, with no example or transport scaffolding.

[![crates.io](https://img.shields.io/crates/v/esp-csi-rs-core.svg)](https://crates.io/crates/esp-csi-rs-core)
[![docs.rs](https://docs.rs/esp-csi-rs-core/badge.svg)](https://docs.rs/esp-csi-rs-core)

> Most users want `esp-csi-rs` instead — it re-exports this crate in full and ships the runnable examples referenced below. Depend on `esp-csi-rs-core` directly only if you are building your own facade or an out-of-tree radio profile.
...
`esp-csi-rs-core` builds on top of Espressif's low-level abstractions to enable easy CSI collection on embedded ESP devices. The crate supports various WiFi modes and network configurations and integrates with the `esp-wifi` and `embassy` async ecosystems.
