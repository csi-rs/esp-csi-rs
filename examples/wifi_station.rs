...
use esp_csi_rs::{
    config::CsiConfig, logging::logging::init_logger, CSINode, EspNowConfig, WifiSnifferConfig,
    WifiStationConfig,
};
...
async fn main(spawner: Spawner) -> ! {
    ...
    wifi_node.init(interfaces, spawner, controller).await;

    // Collect for 5 Seconds
    with_timeout(Duration::from_secs(5000), async {
        loop {
            wifi_node.print_csi_w_metadata().await;
        }
    })
    .await
    .unwrap_err();
