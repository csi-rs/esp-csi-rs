//! ESP-NOW central (Collector) — **HT40 CSI**, MCS7-LGI, 802.11n, multi-chip.
//!
//! Companion central for `esp_now_peripheral_ht40`. Drives an ESP-NOW central in
//! **Collector** mode: it broadcasts control frames (auto-pairing, no hardcoded
//! MACs) and captures the wide CSI from the peripheral's **unicast** HT40
//! replies. The TX PHY is forced to **MCS7 Long-GI** (`RateMcs7Lgi`) on
//! **802.11n** (`Protocol::N`).
//!
...
//!
//! How the link forms: the peripheral receives this central's broadcast, learns
//! its MAC, and replies by **unicast** — the library applies the forced MCS7 /
//! HT40 PHY to that learned peer (a per-peer HT40 rate only applies to a unicast
//! peer; on C5, forcing the broadcast peer is also unsafe). The central collects
//! CSI from those unicast replies.
//!
//! Verify HT40 engaged via the `Subcarriers` field in the stats line: ≥ 100
//! (commonly ~117–128) confirms HT40; ~53/~56 means it fell back to legacy/HT20.
//! 2.4 GHz HT40 can be finicky on some chips — if subcarriers stay narrow, try a
//! different primary/secondary pair (e.g. ch 1 Above, ch 11 Below) or fall back
//! to HT20.
//!
...
//!   cargo esp32c5-defmt --example esp_now_central_ht40   # ch 149 (5 GHz)
//!   cargo esp32c6-defmt --example esp_now_central_ht40   # ch 6   (2.4 GHz)
//!   cargo esp32s3-defmt --example esp_now_central_ht40   # ch 6   (2.4 GHz)
//!   cargo esp32-defmt   --example esp_now_central_ht40   # ch 6   (2.4 GHz)
...
use esp_csi_rs::{
    CSINodeClient, CSINodeHardware, get_dropped_packets_rx, get_pps_rx, get_pps_tx,
    get_total_rx_packets, get_total_tx_packets, log_ln, set_csi_callback,
};
...
// Broadcast control-frame rate (Hz). Matches the peripheral's reply pacing.
const TX_RATE_HZ: u16 = 1000;

// --- Per-chip channel / HT40 secondary selection (must match the peripheral) -
...
// HT40 → ~117–128 subcarriers (csi_data_len / 2); HT20/legacy → ~53/56.
static LAST_SUBCARRIERS: AtomicU32 = AtomicU32::new(0);
...
fn on_csi(packet: &CSIDataPacket) {
    ...
    LAST_SUBCARRIERS.store((packet.csi_data_len / 2) as u32, Ordering::Relaxed);
}
...
async fn stats_task() {
    ...
    loop {
        ...
        log_ln!(
            "RX PPS(avg): {}, TX PPS(avg): {}, RX Hz(inst): {}, TX Hz(inst): {}, RX Total: {}, TX Total: {}, RX Dropped Packets: {}, CSI Packets: {}, Subcarriers: {}, Latest RSSI: {}",
            get_pps_rx(),
            get_pps_tx(),
            rx_rate_hz,
            tx_rate_hz,
            ...
            LAST_SUBCARRIERS.load(Ordering::Relaxed),
...
async fn main(spawner: Spawner) -> ! {
    ...
    log_ln!(
        "Starting EspNow Central (Collector) — channel {}, {}, MCS7-LGI, 802.11n, {} TX/s",
        CHANNEL,
        BANDWIDTH,
        TX_RATE_HZ
    );
    ...
    // HT40 on every chip. `with_ht40` implies `force_phy`, so the central's RX is
    // brought up at 40 MHz and the peripheral's per-peer reply PHY lands as HT40.
    let espnow_cfg = EspNowConfig::default()
        .with_channel(CHANNEL)
        .with_phy_rate(PHY_RATE)
        .with_ht40(SECONDARY);

    // HT40-focused CSI config. The `CsiConfig` shape is per-chip:
    //  - C5: u32 acquire_* fields + `acquire_csi_force_lltf` — drop legacy/HT20/
    //    ACK so the full HT-LTF (~117 subcarriers) isn't masked by ~53 legacy CSI.
    //  - C6: u32 acquire_* fields (no force_lltf).
    //  - ESP32 / S3 / C3: bool LTF flags — use the HT-LTF directly (no merge with
    //    the legacy LTF) so HT40 frames yield the full 40 MHz CSI (~128).
    #[cfg(feature = "esp32c5")]
    ...
    #[cfg(any(feature = "esp32", feature = "esp32s3", feature = "esp32c3"))]
    let csi_cfg = CsiConfig {
        lltf_en: false,
        ltf_merge_en: false,
        ..CsiConfig::default()
    };
    // rust-analyzer fallback (no chip feature): plain default.
    #[cfg(all(
        not(any(
            feature = "esp32",
            feature = "esp32s3",
